var Pizza = {
  ...
  settings : {
    ...
    bar_spacer: 100,
    bar_intervals: 6,
    animation_type: 'backout' // options: backin, backout, bounce, easein, 
  ...
  data : function (legend, options) {
    ...
    $('li', legend).each(function () {
      var segment = $(this);
      var bgColor = $(this).children('.graph-legend-itemColor');

      if (options.data) {
        data.push({
          value: options.data[segment.index()],
          text: segment.data('text'), 
          color: bgColor.css('backgroundColor'),
          segment: segment
        });
      } else {
        data.push({
  ...
  animate : function (el, cx, cy, settings, scale) {
    ...
    el.hover(function (e) {
        ...
        text.animate({
          opacity: 1
          //Changed this to 1 so text always shows, normal is 0
        }, settings.animation_speed);
      });
